...
do
  ...
  if [ -z $link ]
  then
    echo "Sorry, the given query was not found"
  else
    flag=0
  fi
done
...
(lynx -nonumbers -dump $link) > abb.html 2>/dev/null
(grep 'https' abb.html | rev | cut -d "'" -f 2 | rev) > newlinks
 #cat newlinks
(grep -i "/$" newlinks)>dataset

(lynx -nonumbers -dump `cat dataset`) > abc 2>/dev/null
...
echo ""
#echo $disc
lynx -nonumbers -dump $disc |tee description 2>/dev/null
(lynx -nonumbers -dump $b )> fin.csv 2>/dev/null
rm abc
rm acc
rm dataset
rm description
...
if [ $ch -eq 1 ];then
	...
	echo '<number_of_images_per_category> <width> <heigth>'
	read A
	read C
	read D
	num=$A
